        ...
        <div className="flex flex-1 items-center justify-center px-4 sm:px-8 md:px-12 lg:px-16 py-4 sm:py-6 md:py-8 overflow-y-auto">
          <div className="flex flex-col gap-3 sm:gap-4 md:gap-6 w-full max-w-4xl">
            ...
            <BlurFade delay={0.15} duration={0.5}>
              <p className="text-xs sm:text-sm text-[#9CA3AF] uppercase tracking-wider mb-1 sm:mb-2 text-center">-- Simplified --</p>
            </BlurFade>
            {slide.steps.map((step, index) => {
              ...
              return (
                <BlurFade key={index} delay={0.2 + index * 0.15} duration={0.5}>
                  <div className="relative flex items-start gap-3 sm:gap-4 md:gap-6 p-3 sm:p-4 md:p-6 rounded-lg sm:rounded-xl border border-gray-200 bg-white hover:border-[#1A7285]/30 hover:bg-gray-50 transition-all duration-300">
                    {/* Step number */}
                    <div className="flex-shrink-0 flex items-center justify-center w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 lg:w-14 lg:h-14 rounded-full bg-gradient-to-r from-[#1A7285] to-[#1A7285] text-white text-base sm:text-lg md:text-xl lg:text-2xl font-bold">
                      {index + 1}
                    </div>

                    {/* Icon */}
                    <div className="flex-shrink-0 flex items-center justify-center w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 lg:w-14 lg:h-14 rounded-lg bg-gray-100">
                      <Icon className="h-4 w-4 sm:h-5 sm:w-5 md:h-6 md:w-6 lg:h-7 lg:w-7 text-[#1A7285]" />
                    </div>

                    {/* Content */}
                    <div className="flex-1 min-w-0">
                      <h2 className="text-base sm:text-lg md:text-xl lg:text-2xl font-bold text-[#212121] mb-1 sm:mb-2">
                        {step.title}
                      </h2>
