        ...
        <div className="flex flex-1 items-center justify-center px-4 sm:px-8 md:px-12 lg:px-16 py-4 sm:py-6 md:py-8 overflow-y-auto">
          ...
          {slide.directives.map((directive, index) => (
            <BlurFade key={index} delay={0.3 + index * 0.15} duration={0.5}>
              <div className="flex items-center gap-3 sm:gap-4 md:gap-6 p-3 sm:p-4 md:p-6 rounded-xl sm:rounded-2xl border border-[#1A7285]/20 bg-gradient-to-br from-[#1A7285]/5 to-white transition-all duration-300 hover:border-[#1A7285]/30 hover:bg-gradient-to-br hover:from-[#1A7285]/10 hover:to-white hover:scale-[1.02] cursor-default">
                <div className="flex-shrink-0 flex items-center justify-center w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 rounded-full bg-[#1A7285]/10 text-[#1A7285] font-bold text-base sm:text-lg md:text-xl">
                  {index + 1}
                </div>
                <p className="text-sm sm:text-base md:text-xl lg:text-2xl text-[#6B7280] leading-relaxed">
                  {directive}
                </p>
              </div>
            </BlurFade>
